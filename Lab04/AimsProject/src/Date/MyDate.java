...
public class MyDate {
	...
	private final String[] STT = { "first", "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth",
			"ninth", "tenth", "eleventh", "twelfth", "thirdteenth", "fourteenth", "fifteenth", "sixteenth",
			"seventeenth", "eighteenth", "nineteenth", "twentieth", "thirdtieth" };
	...
	// Hàm xử lý chuỗi có dấu "-" (E.g: forty-five), trả về -1 nếu không đọc được
	public int subFunction(String str) {
		...
		if (parts.length == 2) {
			for (i = 20; i < 27; i++) {
				if (parts[0].toLowerCase().equals(digit[i])) {
					result = (i - 18) * 10;
					break;
				}
			}
			...
			for (i = 0; i < 10; i++) {
				if (parts[1].toLowerCase().equals(digit[i])) {
					result = result + i;
					break;
				}
			}
			...
			for (i = 0; i < 27; i++) {
				if (str.toLowerCase().equals(digit[i])) {
					if (i <= 20) {
						return i;
					} else {
						return (i - 18) * 10;
	...
	// Đọc string year, trả về -1 nếu không đọc được
	public int readYear(String stringYear) {
		int i, year = 0, check;
		String[] parts = stringYear.split(" ");
		// Tách chuỗi bằng dấu " ", có 2 trường hợp: có 3 chuỗi (E.g: three hundred
	...
	// Đọc string month, trả về -1 nếu không đọc được
	...
	public int readDay(String stringDay) {
		...
		if (parts.length == 2) {
			...
			for (i = 0; i < 9; i++) {
				if (parts[1].toLowerCase().equals(STT[i])) {
					result = result + i +1;
					break;
				}
			}
			...
			for (i = 0; i < 21; i++) {
				if (stringDay.toLowerCase().equals(STT[i])) {
					if (i <= 19) {
						return i + 1;
					} else {
						return (i - 17) * 10;
	...
	// In ngày tháng năm với các format khác nhau do người dùng chọn trong console
