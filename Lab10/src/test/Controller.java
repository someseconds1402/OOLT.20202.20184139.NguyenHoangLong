...
public class Controller implements Initializable {
	...
	private static Order newOrder = createNewOrder();
	private boolean check = false;
	private boolean checkLuckyItem = false;
	private int index;
	...
	private static Order createManageOrder() {
		...
		Order managerOrder = createNewOrder();
...
//			removeEventListening();
	...
	public void removeItemsEvent(ActionEvent e) {
		ObservableList<Media> List3 = FXCollections.observableArrayList();
		int count = 0;
		for(int i = 0; i < List2.size(); i++) {
			Media item = List2.get(i);
			if(item.getSelected().isSelected()) {
		...
		if(count == 0) {
			...
			for(int i = 0; i < List3.size(); i++) {
				newOrder.removeMedia(List3.get(i));
			}
			dialog.setTitle("MESSAGE!");
	...
	// Button 'Lucky Items' eventHandle
	
	@FXML private AnchorPane luckyItem;
	@FXML private TextField priceField;
	@FXML private Button OKButtonTask5;
	@FXML private Label warningTask5;
	
	public void luckyItemEvent(ActionEvent e) {
		...
		if(check) {
			...
			removeItems.setVisible(false);
			showOrder.setVisible(false);
			luckyItem.setVisible(true);
			
			if(newOrder.getSize() < 4) {
				
				priceField.setEditable(false);
				warningTask5.setText("You can get lucky item if yourOrder.length > 3");
				warningTask5.setVisible(true);
				OKButtonTask5.setDisable(true);				
			} else {
				if(checkLuckyItem) {
					priceField.setEditable(false);
					warningTask5.setText("You have got lucky items!");
					warningTask5.setVisible(true);
					OKButtonTask5.setDisable(true);
				} else {
					priceField.setEditable(true);
					warningTask5.setVisible(false);
					OKButtonTask5.setDisable(false);
				}
			}
	...
	public void getLuckyItem() {
		String inputText = priceField.getText();

			try {
				float inputPrice = Float.parseFloat(inputText);
				boolean c;
				try {
					Media luckyItem = managerOrder.getLuckyItem(inputPrice);
					warningTask5.setVisible(false);
					try {
						c = newOrder.isAdded(luckyItem);
						if(c) {
							warningTask5.setText("Oops, Unlucky! Try again!");
							warningTask5.setVisible(true);
						} else {
							if(luckyItem == null) {
								System.out.println(1);
							}
							newOrder.addMedia(luckyItem);
							newOrder.findItem(luckyItem.getTitle()).setCost(0.0f);
							checkLuckyItem = true;
							Dialog<String> dialog = new Dialog<>();
							dialog.setTitle("Congratulate!");
							dialog.setHeaderText("OMG! YO BRUH! YOU ARE VERY LUCKY GUY!");
							dialog.setContentText("You have got a lucky item! Go to \"Show Order\" to see detail.");
							dialog.getDialogPane().getButtonTypes().add(ButtonType.OK);
							dialog.show();
						}
					} catch (Exception e) {
				...
				warningTask5.setText("Invalid input! Please try again!");
				warningTask5.setVisible(true);
			}
